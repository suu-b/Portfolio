...
export default function ProjectCard({ imageSrc, projectName, projectDescription, techStack, liveLink, repoLink }) {
    return (
        <div className="bg-white dark:bg-black mb-8 mx-auto border border-slate-300 dark:border-slate-900 border-2 rounded-lg shadow-lg p-3 lg:p-5">
            ...
            <div className="flex justify-start flex-wrap items-center my-3">{techStack.map(skill => <div key={skill.skillName} className={`text-center py-1 px-2 m-1 rounded text-[13px] lg:text-sm pr-3 ${skills[skill].bgColor} border ${skills[skill].borderColor} ${skills[skill].textColor}`}>{skills[skill].skillName}</div>)} </div>
            <div className='flex'>
